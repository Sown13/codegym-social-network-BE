...
public class GroupController {
    ...
    @PostMapping("/user/{sourceUserId}/{targetUserId}")
    public ResponseEntity<?> createMessageGroupBetween2user(@PathVariable Long sourceUserId,@PathVariable Long targetUserId){
        try {

            return new ResponseEntity<>(   groupService.createChatRoom(sourceUserId,targetUserId),HttpStatus.OK);
        }
        catch (Exception e) {
            return new ResponseEntity<>("Failed to create group: " + e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
